...
<head>
    ...
    <title>@yield('title', 'Dashboard') — Edu Ular Tangga</title>
        ...
        <aside id="sidebar" class="fixed inset-y-0 left-0 z-40 w-64 bg-gradient-to-b from-primary-800 to-primary-900 text-white transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out">
            ...
            <nav class="mt-4 px-3 space-y-1">
                ...
                @elseif(auth()->user()->role === 'guru')
                    ...
                    <a href="{{ route('guru.soal.index') }}" class="sidebar-link {{ request()->routeIs('guru.soal.*') ? 'active' : '' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                        Bank Soal
                    </a>
                    <a href="{{ route('guru.paket-permainan.index') }}" class="sidebar-link {{ request()->routeIs('guru.paket-permainan.*') ? 'active' : '' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/></svg>
                        Paket Permainan
                    </a>
                    <a href="{{ route('guru.laporan.index') }}" class="sidebar-link {{ request()->routeIs('guru.laporan.*') ? 'active' : '' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                        Laporan
                    </a>
                @elseif(auth()->user()->role === 'siswa')
                    ...
                    <a href="{{ route('siswa.game.index') }}" class="sidebar-link {{ request()->routeIs('siswa.game.*') ? 'active' : '' }}">
                    ...
                    <a href="{{ route('siswa.riwayat.index') }}" class="sidebar-link {{ request()->routeIs('siswa.riwayat.*') ? 'active' : '' }}">
            ...
            <div class="absolute bottom-0 left-0 right-0 p-4 border-t border-primary-700/50">
                ...
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit" class="w-full flex items-center gap-2 px-3 py-2 text-sm text-primary-200 hover:bg-primary-700 rounded-lg transition-colors">
        ...
        <div class="flex-1 lg:ml-64">
            ...
            <header class="sticky top-0 z-30 bg-white/80 backdrop-blur-lg border-b border-gray-200/60">
                <div class="flex items-center justify-between px-4 py-3 lg:px-6">
                    ...
                    <div class="flex items-center gap-3">
                        <a href="{{ route('landing') }}" class="text-sm text-gray-500 hover:text-primary-600 transition-colors">← Beranda</a>
                    </div>
                </div>
            </header>
